...
- SUM 함수 예시: B2부터 B10까지의 모든 값을 더해서 B11에 합계 표시
  {"command_type": "sum", "target_range": "B11", "parameters": ["B2:B10"]}

- AVERAGE 함수 예시: C2부터 C20까지 점수의 평균을 계산해서 C21에 표시 (빈 셀 제외하고 계산)
  {"command_type": "average", "target_range": "C21", "parameters": ["C2:C20"]}

- COUNT 함수 예시: D2부터 D15까지 범위에서 숫자가 입력된 셀의 개수를 D16에 표시
  {"command_type": "count", "target_range": "D16", "parameters": ["D2:D15"]}

- MAX 함수 예시: E2부터 E50까지 범위에서 가장 큰 값을 찾아서 E51에 최댓값 표시
  {"command_type": "max", "target_range": "E51", "parameters": ["E2:E50"]}

- MIN 함수 예시: F2부터 F30까지 범위에서 가장 작은 값을 찾아서 F31에 최솟값 표시
  {"command_type": "min", "target_range": "F31", "parameters": ["F2:F30"]}

- SET_VALUE 함수 예시: A1 셀에 "제품명"이라는 텍스트를 입력하여 헤더 설정
  {"command_type": "set_value", "target_range": "A1", "parameters": ["제품명"]}

- CLEAR 함수 예시: B5부터 D10까지 범위의 모든 내용을 지워서 데이터 초기화
  {"command_type": "clear", "target_range": "B5:D10", "parameters": []}

- MERGE 함수 예시: A1부터 C1까지 3개 셀을 병합해서 제목 영역으로 만들기
  {"command_type": "merge", "target_range": "A1:C1", "parameters": []}

- UNMERGE 함수 예시: 이전에 병합된 A1:C1 범위를 다시 개별 셀로 분리하기
  {"command_type": "unmerge", "target_range": "A1:C1", "parameters": []}


논리 함수 상세 예시
# IF 함수 - 단순 조건 판정
- 기본 합격/불합격: B2 점수가 60점 이상이면 "합격", 미만이면 "불합격"을 C2에 표시
  {"command_type": "if", "target_range": "C2", "parameters": ["B2>=60", "합격", "불합격"]}

- 나이 기준 분류: D2 나이가 18세 이상이면 "성인", 미만이면 "미성년자"를 E2에 표시
  {"command_type": "if", "target_range": "E2", "parameters": ["D2>=18", "성인", "미성년자"]}

- 재고 관리: F2 재고수량이 10개 미만이면 "주문필요", 이상이면 "충분"을 G2에 표시
  {"command_type": "if", "target_range": "G2", "parameters": ["F2<10", "주문필요", "충분"]}

- 급여 계산: H2 근무시간이 40시간 초과면 초과수당 적용(시급*1.5), 아니면 기본시급을 I2에 계산
  {"command_type": "if", "target_range": "I2", "parameters": ["H2>40", "H2*15000*1.5", "H2*15000"]}

# AND 함수 - 모든 조건이 참일 때
- 장학금 대상자: J2 성적이 90점 이상이고 K2 출석률이 95% 이상일 때 TRUE를 L2에 표시
  {"command_type": "and", "target_range": "L2", "parameters": ["J2>=90", "K2>=95"]}

- 할인 대상 상품: M2 가격이 10만원 이상이고 N2 카테고리가 "전자제품"일 때 TRUE를 O2에 표시
  {"command_type": "and", "target_range": "O2", "parameters": ["M2>=100000", "N2=\"전자제품\""]}

- 대출 승인 조건: P2 연봉이 3000만원 이상이고 Q2 신용등급이 1~3등급일 때 TRUE를 R2에 표시
  {"command_type": "and", "target_range": "R2", "parameters": ["P2>=30000000", "Q2<=3", "Q2>=1"]}

- 우수 사원: S2 근무년수가 3년 이상이고 T2 평가점수가 85점 이상이고 U2 지각횟수가 5회 미만일 때 TRUE를 V2에 표시
  {"command_type": "and", "target_range": "V2", "parameters": ["S2>=3", "T2>=85", "U2<5"]}

# OR 함수 - 하나라도 참이면 됨
- 특별 고객: W2가 "VIP"이거나 X2 구매금액이 100만원 이상일 때 TRUE를 Y2에 표시
  {"command_type": "or", "target_range": "Y2", "parameters": ["W2=\"VIP\"", "X2>=1000000"]}

- 긴급 처리: Z2 우선순위가 "긴급"이거나 AA2 마감일이 오늘 이전일 때 TRUE를 AB2에 표시
  {"command_type": "or", "target_range": "AB2", "parameters": ["Z2=\"긴급\"", "AA2<TODAY()"]}

- 휴일 근무: AC2가 "토요일"이거나 AD2가 "일요일"이거나 AE2가 "공휴일"일 때 TRUE를 AF2에 표시
  {"command_type": "or", "target_range": "AF2", "parameters": ["AC2=\"토요일\"", "AD2=\"일요일\"", "AE2=\"공휴일\""]}

# IFERROR 함수 - 오류 처리
- 나눗셈 오류 방지: AG2를 AH2로 나눈 결과를 AI2에 표시하되, 0으로 나누기 등 오류 시 "계산불가" 표시
  {"command_type": "iferror", "target_range": "AI2", "parameters": ["AG2/AH2", "계산불가"]}

- VLOOKUP 오류 처리: AJ2 학번으로 학생정보를 찾되, 없는 학번이면 "미등록학생"을 AK2에 표시
  {"command_type": "iferror", "target_range": "AK2", "parameters": ["VLOOKUP(AJ2,학생명단!A:C,2,0)", "미등록학생"]}

- 수식 오류 대응: AL2*AM2*AN2 곱셈 결과를 AO2에 표시하되, 텍스트 등으로 인한 오류 시 0 표시
  {"command_type": "iferror", "target_range": "AO2", "parameters": ["AL2*AM2*AN2", 0]}

# IFNA 함수 - #N/A 오류 전용 처리
- 검색 결과 없음: AP2 제품코드로 제품명을 찾되, 등록되지 않은 제품이면 "신제품"을 AQ2에 표시
  {"command_type": "ifna", "target_range": "AQ2", "parameters": ["VLOOKUP(AP2,제품목록!A:B,2,0)", "신제품"]}

- 매칭 데이터 없음: AR2 고객번호로 고객등급을 찾되, 신규고객이면 "일반등급"을 AS2에 표시
  {"command_type": "ifna", "target_range": "AS2", "parameters": ["INDEX(고객정보!C:C,MATCH(AR2,고객정보!A:A,0))", "일반등급"]}

# IFS 함수 - 다중 조건 처리
- 성적 등급: AT2 점수에 따라 90이상="A", 80이상="B", 70이상="C", 60이상="D", 나머지="F"를 AU2에 표시
  {"command_type": "ifs", "target_range": "AU2", "parameters": ["AT2>=90", "A", "AT2>=80", "B", "AT2>=70", "C", "AT2>=60", "D", "TRUE", "F"]}

- 배송비 계산: AV2 주문금액에 따라 10만원이상=무료, 5만원이상=2500원, 3만원이상=3500원, 나머지=5000원을 AW2에 표시
  {"command_type": "ifs", "target_range": "AW2", "parameters": ["AV2>=100000", 0, "AV2>=50000", 2500, "AV2>=30000", "3500", "TRUE", 5000]}

- 근무 형태: AX2 근무시간에 따라 40시간이상="정규직", 20시간이상="파트타임", 10시간이상="아르바이트", 나머지="인턴"을 AY2에 표시
  {"command_type": "ifs", "target_range": "AY2", "parameters": ["AX2>=40", "정규직", "AX2>=20", "파트타임", "AX2>=10", "아르바이트", "TRUE", "인턴"]}

조건부 연산 함수 상세 예시
# COUNTIF 함수 - 조건부 개수 세기
- 성별 통계: AZ2:AZ100 범위에서 "남성"인 직원 수를 BA101에 계산
  {"command_type": "countif", "target_range": "BA101", "parameters": ["AZ2:AZ100", "\"남성\""]}

- 합격자 수: BB2:BB50 성적에서 80점 이상인 학생 수를 BC51에 계산
  {"command_type": "countif", "target_range": "BC51", "parameters": ["BB2:BB50", ">=80"]}

- 재고 부족: BD2:BD30 재고량에서 10개 미만인 상품 개수를 BE31에 계산
  {"command_type": "countif", "target_range": "BE31", "parameters": ["BD2:BD30", "<10"]}

- 특정 날짜: BF2:BF200 입사일에서 2023년도 입사자 수를 BG201에 계산
  {"command_type": "countif", "target_range": "BG201", "parameters": ["BF2:BF200", ">=2023-01-01"]}

- 부서별 인원: BH2:BH80 부서명에서 "개발팀"에 속한 직원 수를 BI81에 계산
  {"command_type": "countif", "target_range": "BI81", "parameters": ["BH2:BH80", "\"개발팀\""]}

# SUMIF 함수 - 조건부 합계
- 부서별 급여: BJ2:BJ50 부서에서 "영업부"인 직원들의 BK2:BK50 급여 합계를 BL51에 계산
  {"command_type": "sumif", "target_range": "BL51", "parameters": ["BJ2:BJ50", "\"영업부\"", "BK2:BK50"]}

- 고득점 합계: BM2:BM40 과목에서 "수학"인 BN2:BN40 점수들의 합계를 BO41에 계산
  {"command_type": "sumif", "target_range": "BO41", "parameters": ["BM2:BM40", "\"수학\"", "BN2:BN40"]}

- 매출 집계: BP2:BP100 지역이 "서울"인 BQ2:BQ100 매출액 합계를 BR101에 계산
  {"command_type": "sumif", "target_range": "BR101", "parameters": ["BP2:BP100", "\"서울\"", "BQ2:BQ100"]}

- 기간별 매출: BS2:BS365 날짜가 2024년 1월인 BT2:BT365 매출 합계를 BU366에 계산
  {"command_type": "sumif", "target_range": "BU366", "parameters": ["BS2:BS365", ">=2024-01-01", "BT2:BT365"]}

# AVERAGEIF 함수 - 조건부 평균
- 성별 평균점수: BV2:BV60 성별이 "여성"인 BW2:BW60 점수들의 평균을 BX61에 계산
  {"command_type": "averageif", "target_range": "BX61", "parameters": ["BV2:BV60", "\"여성\"", "BW2:BW60"]}

- 경력별 연봉: BY2:BY80 경력이 5년 이상인 BZ2:BZ80 연봉들의 평균을 CA81에 계산
  {"command_type": "averageif", "target_range": "CA81", "parameters": ["BY2:BY80", ">=5", "BZ2:BZ80"]}

- 지역별 기온: CB2:CB120 지역이 "부산"인 CC2:CC120 기온들의 평균을 CD121에 계산
  {"command_type": "averageif", "target_range": "CD121", "parameters": ["CB2:CB120", "\"부산\"", "CC2:CC120"]}

### 복합 조건 예시 (실무에서 자주 사용)
# IF + AND 조합 - 복수 조건 만족시
- 보너스 지급: CE2 연봉이 5000만원 이상이고 CF2 평가가 "우수"일 때 연봉의 10%, 아니면 5%를 CG2에 계산
  {"command_type": "if", "target_range": "CG2", "parameters": ["AND(CE2>=50000000,CF2=\"우수\")", "CE2*0.1", "CE2*0.05"]}

# IF + OR 조합 - 여러 조건 중 하나 만족시
- 할인 적용: CH2가 "VIP"이거나 CI2 구매금액이 50만원 이상일 때 10% 할인, 아니면 할인없음을 CJ2에 계산
  {"command_type": "if", "target_range": "CJ2", "parameters": ["OR(CH2=\"VIP\",CI2>=500000)", "CI2*0.9", "CI2"]}

# 중첩 IF 함수 - 3단계 조건
- 배송 방법: CK2 무게가 30kg 이상이면 "화물배송", 10kg 이상이면 "택배", 나머지는 "일반우편"을 CL2에 표시
  {"command_type": "if", "target_range": "CL2", "parameters": ["CK2>=30", "화물배송", "IF(CK2>=10,\"택배\",\"일반우편\")"]}

# COUNTIFS와 유사한 동작 (COUNTIF + 논리함수 조합)
- 조건부 카운트: CM2:CM100에서 "A등급"이고 CN2:CN100에서 "완료"인 항목 수를 계산하기 위한 보조열 CO2 생성
  {"command_type": "if", "target_range": "CO2", "parameters": ["AND(CM2=\"A등급\",CN2=\"완료\")", 1, 0]}

### 검색 및 참조
- VLOOKUP 함수 예시: I2 학번을 A2:C100 학생명단에서 찾아 3번째 열(이름)을 J2에 표시
  {"command_type": "vlookup", "target_range": "J2", "parameters": ["I2", "A2:C100", 3, false]}

- HLOOKUP 함수 예시: K2 월을 A1:M5 매출표에서 찾아 2번째 행(매출액)을 L2에 표시
  {"command_type": "hlookup", "target_range": "L2", "parameters": ["K2", "A1:M5", 2, false]}

- INDEX 함수 예시: M2:O20 범위에서 3번째 행, 2번째 열에 있는 값을 P2에 표시
  {"command_type": "index", "target_range": "P2", "parameters": ["M2:O20", 3, 2]}

- MATCH 함수 예시: Q2 값이 R2:R50 범위에서 몇 번째 위치에 있는지를 S2에 표시
  {"command_type": "match", "target_range": "S2", "parameters": ["Q2", "R2:R50", 0]}

- XLOOKUP 함수 예시: T2 제품코드를 U2:U30에서 찾아 V2:V30의 제품명을 W2에 표시 (최신 검색함수)
  {"command_type": "xlookup", "target_range": "W2", "parameters": ["T2", "U2:U30", "V2:V30"]}

- FILTER 함수 예시: X2:Z50 데이터에서 Y열이 "A등급"인 행들만 필터링해서 AA2부터 표시
  {"command_type": "filter", "target_range": "AA2", "parameters": ["X2:Z50", "Y2:Y50=\"A등급\""]}

- UNIQUE 함수 예시: AB2:AB100 범위에서 중복 제거된 고유한 값들만 AC2부터 세로로 표시
  {"command_type": "unique", "target_range": "AC2", "parameters": ["AB2:AB100"]}

### 통계 함수
- MEDIAN 함수 예시: AD2:AD30 점수들의 중간값(50퍼센타일)을 계산해서 AE31에 표시
  {"command_type": "median", "target_range": "AE31", "parameters": ["AD2:AD30"]}

- MODE 함수 예시: AF2:AF40 데이터에서 가장 자주 나타나는 값(최빈값)을 AG41에 표시
  {"command_type": "mode", "target_range": "AG41", "parameters": ["AF2:AF40"]}

- STDEV 함수 예시: AH2:AH35 숫자들의 표준편차를 계산해서 데이터의 분산 정도를 AI36에 표시
  {"command_type": "stdev", "target_range": "AI36", "parameters": ["AH2:AH35"]}

- RANK 함수 예시: AJ2 학생의 점수가 AJ2:AJ50 전체 범위에서 몇 등인지를 AK2에 표시 (0=내림차순)
  {"command_type": "rank", "target_range": "AK2", "parameters": ["AJ2", "AJ2:AJ50", 0]}

### 텍스트 함수
- CONCATENATE 함수 예시: AL2(성)과 AM2(이름)을 합쳐서 "홍길동" 형태로 AN2에 표시
  {"command_type": "concatenate", "target_range": "AN2", "parameters": ["AL2", "AM2"]}

- & 함수 예시: AO2와 AP2 문자열을 " - " 구분자와 함께 연결해서 AQ2에 표시
  {"command_type": "&", "target_range": "AQ2", "parameters": ["AO2", "\" - \"", "AP2"]}

- LEFT 함수 예시: AR2 텍스트에서 왼쪽부터 3글자만 추출해서 AS2에 표시 (예: "홍길동"→"홍길동")
  {"command_type": "left", "target_range": "AS2", "parameters": ["AR2", 3]}

- RIGHT 함수 예시: AT2 전화번호에서 오른쪽 끝 4자리만 추출해서 AU2에 표시
  {"command_type": "right", "target_range": "AU2", "parameters": ["AT2", 4]}

- MID 함수 예시: AV2 문자열에서 3번째 위치부터 2글자를 추출해서 AW2에 표시
  {"command_type": "mid", "target_range": "AW2", "parameters": ["AV2", 3, 2]}

- LEN 함수 예시: AX2 텍스트의 전체 글자 수(공백 포함)를 계산해서 AY2에 표시
  {"command_type": "len", "target_range": "AY2", "parameters": ["AX2"]}

- SUBSTITUTE 함수 예시: AZ2 텍스트에서 "구버전"을 "신버전"으로 모두 바꿔서 BA2에 표시
  {"command_type": "substitute", "target_range": "BA2", "parameters": ["AZ2", "구버전", "신버전"]}

- TRIM 함수 예시: BB2 텍스트의 앞뒤 공백과 중간의 연속 공백을 제거해서 BC2에 표시
  {"command_type": "trim", "target_range": "BC2", "parameters": ["BB2"]}

- UPPER 함수 예시: BD2 영문 텍스트를 모두 대문자로 변환해서 BE2에 표시 ("hello"→"HELLO")
  {"command_type": "upper", "target_range": "BE2", "parameters": ["BD2"]}

- LOWER 함수 예시: BF2 영문 텍스트를 모두 소문자로 변환해서 BG2에 표시 ("WORLD"→"world")
  {"command_type": "lower", "target_range": "BG2", "parameters": ["BF2"]}

### 기타 함수
- ROUND 함수 예시: BH2 소수값을 소수점 둘째 자리까지 반올림해서 BI2에 표시 (예: 3.14159→3.14)
  {"command_type": "round", "target_range": "BI2", "parameters": ["BH2", 2]}

- ISBLANK 함수 예시: BJ2 셀이 비어있는지 확인해서 TRUE/FALSE를 BK2에 표시
  {"command_type": "isblank", "target_range": "BK2", "parameters": ["BJ2"]}
