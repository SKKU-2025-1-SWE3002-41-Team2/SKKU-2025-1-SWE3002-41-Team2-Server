...
def test_modify_session_success():
    ...
    result = chat.modify_session(1, "New Name", mock_db)
...
def test_modify_session_not_found():
    ...
    with pytest.raises(SessionNotFoundException):
        chat.modify_session(1, "New Name", mock_db)


# [CREATE] create_session 호출 시 내부 흐름이 정상적으로 작동하는지 테스트
@patch("app.services.chat.validate_user_exists")
@patch("app.services.chat.save_message_and_response")
def test_create_session_flow(mock_save_msg, mock_validate_user):
    ...
    with patch("app.services.chat.ChatSession", return_value=fake_session):
        result = chat.create_session(1, "Hello", b"excel-bytes", mock_db)
...
@patch("app.services.chat.insert_message_to_db")
@patch("app.services.chat.get_llm_response")
@patch("app.services.chat.process_excel_with_commands")
@patch("app.services.chat.update_session_summary")
@patch("app.services.chat.upsert_chat_sheet")
def test_save_message_and_response_flow(
    ...
    result = chat.save_message_and_response(1, "Hi", b"old-bytes", mock_db)
